...
from .base import Auth, Cancelled, Context, Item, Source, TooManyTiles
from .custom import custom_sources
from .faa import faa_sources
from .local import local_sources
from .maxar import maxar_sources
from .services import public_service_sources

__all__ = ["Auth", "Cancelled", "Context", "Item", "Source", "TooManyTiles", "registry", "refresh"]
...
def refresh(settings) -> dict[str, Source]:
    global _registry
    with _lock:
        srcs = [*faa_sources(), *public_service_sources(), *maxar_sources(),
               *local_sources(settings), *custom_sources(settings)]
